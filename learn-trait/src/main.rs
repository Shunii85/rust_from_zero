...
fn main() {
    ...
    // 型をより抽象的に表すための型で、トレイトを用いて存在型を表現する。

    // [impl Fn(i32) -> i32]という存在型のクロージャを返す
    ...
    println!("{:?}", get_type_id(&f));
    println!("{:?}", get_type_id(&g));

    // Rustでは、コンパイル時にクロージャにユニークな型を割り当てるため、型を明示的に書くことができない
    // 存在型が役に立つというわけ
    println!("{}", get_type_id(&f) == get_type_id(&g))

    // 存在型は静的ディスパッチ

    // よって複数の型を返す関数はコンパイルエラーになる
